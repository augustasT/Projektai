...
<p font-size:250%;><b><i>Bendri rezultatai </b></i></p>
	...
	<table aling:"center" class="table-striped">
		...
		<c:forEach items="${userList}" var="user">
			<tr>
				...
				<c:if test="${question.getId()>4 && question.getUserId()==user.getId()}">
					<td>${question.getQuestion()}</td>					
				<c:forEach items="${answerList}" var="answer">
				
				<c:if test="${answer.getQuestionId()==question.getId() && answer.getUserId()==user.getId()}">
